...
Vector::Vector(int x1, int y1, int x2, int y2, int x3, int y3, QGraphicsRectItem* item)
    :x1(y1), x2(y2), x3(y3), y1(y1), y2(y2), y3(y3)
{
    ...
    jigolo = new QGraphicsLineItem(400, 400, x1 + x2 - 400 ,y1 + y3 - 400, item);
    genX = x1 + x2 - 400;
    genY = y1 + y3 - 400;
    QPen pen;
    pen.setWidth(8);
...
#include <cmath>
double Vector::getLong()
{
    qDebug()<<sqrt(((x1 + x2 - 400) * (x1 + x2 - 400)) + ((y1 + y3 - 400) * (y1 + y3 - 400)));
    return sqrt(((x1 + x2 - 400) * (x1 + x2 - 400)) + ((y1 + y3 - 400) * (y1 + y3 - 400)));
}
